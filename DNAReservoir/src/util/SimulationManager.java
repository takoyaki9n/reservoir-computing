...
public class SimulationManager {
	...
	private static HashMap<String, Double> test(HashMap<String, MyOLSMultipleLinearRegression> regressions) {
		...
		for (String taskId : tasks.keySet()) {
			...
			exportResult(caseDir + "/result_" + task.id + ".dat", task, regression.calculateEstimatedValues(resultTrimed));
		}

		return nrmses;
	}

	private static void exportResult(String resultFileName, Task task, double[] values) {
		try {
			FileWriter writer = new FileWriter(resultFileName);
			for (int t = 0; t< simulationTime; t++) {
				double v = 0.0;
				if (task.start <= t && t < task.end) v = values[t - task.start];
				writer.write(v + "\n");
			}
			writer.close();
		} catch (IOException e1) {
